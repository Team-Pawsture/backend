...
## BL-10 · `duration_seconds` 응답 필드 제거 검토 — ✅ 완료 (2026-05-20)

**처리**: 나경님 미사용 확인 → 응답에서 `duration_seconds` 키 제거. 더불어 네이버 Directions 호출 자체를 제거하고 `distance_meters` 를 Haversine 직선 거리로 자체 계산. `kakao_distance` 내부 키도 응답에서 제거. 응답 시간 약 5배 단축 (GET /hospitals ~0.79s → ~0.17s).

---

## BL-11 · 병원 추천 점수 임계값 재검토 (운전 거리 → 직선 거리 전환 후)

**우선순위**: P3
**트리거**: 운영 환경 사용자 피드백 수집 후 (1차 배포 후 2~4주).
**발견 시점**: 2026-05-20 (BL-10 완료 시점)

**현황**
- `_compute_recommend_score` 의 거리 점수 임계값(≤200m / ≤500m / ≤1000m) 은 원래 네이버 Directions 운전 거리 기준이었음
- BL-10 으로 `distance_meters` 가 **Haversine 직선 거리**로 변경됨
- 직선 거리 ≈ 운전 거리 × 0.65~0.8 (일반적으로 운전 거리가 더 김)
- 즉 같은 사용자가 같은 병원을 보더라도 직선 거리 기준이라 더 짧게 측정 → 거리 점수 상위 구간(+30/+20)에 더 많이 들어옴

**영향**
- 거리 가중치 인플레이션 가능성 → 다른 가중치(전문분야 +40, 슬개골 기왕력 +15) 대비 거리 영향력이 상대적으로 커짐
- 사용자 체감상 "병원 추천 = 그냥 거리순"으로 보일 위험

**후보 액션**
- 사용자 피드백 + 로그 데이터로 실제 거리 분포 측정
- 임계값을 (150m / 350m / 700m) 등으로 좁히는 옵션 고려
- 또는 점수 배점 자체 조정 (거리 30/20/10 → 20/15/10 등)

**관련 코드**
- `app/routers/hospitals.py:_compute_recommend_score` (임계값 위치)
- `app/routers/hospitals.py:_build_nearby_hospitals` (직선 거리 계산 지점)

**우선순위**: P3
**트리거**: 나경님(프론트)께 사용 여부 확인 후 결정.
**발견 시점**: 2026-05-20 (operation_status 리네임 작업 중)

**현황**
- `GET /hospitals` 및 `POST /hospitals/recommend` 응답의 각 hospital 항목에 `duration_seconds`(네이버 Directions 의 운전 시간 추정값) 포함
- `distance_meters` 와 함께 항상 동반 노출됨
- 거리 점수 계산은 `distance_meters` 만 사용 (`duration_seconds` 는 미사용)
- 프론트 UI 에서 실제로 표시되는지 확인 필요

**영향**
- 미사용이면 네이버 Directions API 호출 자체를 줄여 응답 시간 단축 가능
  (`calculate_distances_parallel` 호출 제거 또는 단순 Haversine 으로 대체)
- 응답 크기 미세 감소

**후보 액션**
- 나경님께 표시 여부 확인
- 미사용 확인 시:
  · 응답에서 `duration_seconds` 키 제거
  · 거리 계산을 Haversine 만으로 단순화 검토 (정확도 vs 속도 트레이드오프)
  · 네이버 클라우드 API 비용/쿼터도 절감

**관련 코드**
- `app/utils/naver_api.py:calculate_distances_parallel`
- `app/routers/hospitals.py:_build_nearby_hospitals` (현재 호출 지점)
- `app/schemas/hospital.py:HospitalItem.duration_seconds`
